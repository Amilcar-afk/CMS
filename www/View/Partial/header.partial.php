<header class="header <?= (isset($backgroundColor) && !preg_match('/^#[a-f0-9]{6}$/i', $backgroundColor)) ? $backgroundColor :'' ?> <?= (isset($radius[0]) && $radius[0]->getValue() == 'right_angle' )? '' : 'header--radius' ?>"
    ...
    <div class="body-go  <?= (isset($backgroundColor) && !preg_match('/^#[a-f0-9]{6}$/i', $backgroundColor)) ? $backgroundColor :'' ?>" <?= (isset($backgroundColor) && preg_match('/^#[a-f0-9]{6}$/i', $backgroundColor)) ? "style='background-color:".$backgroundColor."'" :'' ?>>
        <a id="logo" href="/"><img style="box-shadow: unset" src="<?= (isset($logo[0])) ? $logo[0]->getPath() :'/style/images/logo_myfolio.png'  ?>" alt="logo"></a>
        ...
        <nav class="navbar">
            <ul class="menu">
                ...
                <?php if(isset($_SESSION['Auth']->lastname) && $_SESSION['Auth']->firstname): ?>
                    <li><a class="<?= (isset($btnTextColor) && !preg_match('/^#[a-f0-9]{6}$/i', $btnTextColor)) ? $btnTextColor :'' ?>" <?= (isset($btnTextColor) && preg_match('/^#[a-f0-9]{6}$/i', $btnTextColor)) ? "style='color:".$btnTextColor."'" :'' ?> href="/<?= (isset($_SESSION['Auth']) && $_SESSION['Auth']->rank == 'admin')?'dashboard':'conversations' ?>"><?= $_SESSION['Auth']->lastname . " " . $_SESSION['Auth']->firstname ?></a></li>
                <?php else: ?>
